...
import pandas as pd
import imp
seqtools = imp.load_source('seqtools', '../project_pylib/seqtools.py')
sequnwrap = seqtools.sequnwrap
parse_nested_dicts = seqtools.parse_nested_dicts
seq2html = imp.load_source('seq2html', '../project_pylib/seq2html.py')
seq2pretty_html = seq2html.seq2pretty_html
write_css_stylesheet = seq2html.write_css_stylesheet
alignlib = imp.load_source('align', '../project_pylib/align.py')
run_clustalo = alignlib.run_clustalo
score_aln = alignlib.score_aln
    ...
    for i in range(len(alignment)):
        ...
        if aa_css_class_list != None:
            if aa_css_class_list[i] != None:
                prettyseq = seq2pretty_html(alignment[i], aa_css_class_list=aa_css_class_list[i])
            else:
                prettyseq = seq2pretty_html(alignment[i])
        else:
            prettyseq = seq2pretty_html(alignment[i])
...
def process_target(t):
    ...
    UniProt_canonseq = sequnwrap( DB_entry.findtext('UniProt/isoforms/canonical_isoform/sequence') )
    target_domain_seq = sequnwrap( DB_domain.findtext('sequence') )
    ...
    target_manual_exception = parse_nested_dicts(manual_exceptions, [target, 'behavior'])
    if target_manual_exception != None:
        if target_manual_exception == 'skip':
            manual_exception_comment = parse_nested_dicts(manual_exceptions, [target, 'comment'])
    ...
    # remove 'X' residues
    PDB_seqs = { seq_node.getparent().getparent().getparent().get('ID') + '_' + seq_node.getparent().getparent().get('ID') : sequnwrap(seq_node.text.replace('X', '')) for seq_node in PDB_matching_seq_nodes }
    ...
    PDB_seqs_aln_vs_UniProt_conflicts = { seq_node.getparent().getparent().getparent().get('ID') + '_' + seq_node.getparent().getparent().get('ID') : sequnwrap(seq_node.getparent().find('sequence_aln_conflicts').text.replace('x','-')) for seq_node in PDB_matching_seq_nodes }
    ...
    aligned_seqs = run_clustalo(alignment_IDs, pre_alignment_seqs)
    ...
    for i, PDB_chain_ID in enumerate(PDB_seqs.keys()):
        ...
        alignment_override = parse_nested_dicts(manual_exceptions, [target, PDB_ID, 'alignment_override'])
    ...
    TEV_cleaved_Nterm_regex = '^(gh|ga|gs|ghm|gam|gsm|sm)g{0,1}[sd]{0,1}[A-Z]+[A-Z]{30}'
    ...
    for i in range(len(PDB_construct_seqs_aligned)):
        ...
        manual_exception_behavior = parse_nested_dicts(manual_exceptions, [target, PDB_entry_ID, 'authenticity_score', 'behavior'])
    ...
    for i in range(len(PDB_construct_seqs_aligned)):
        ...
        aln_scores[i] = 0 - score_aln(UniProt_canon_seq_target_domain_seq, PDB_seq_target_domain_seq) # subtract from 0 to reverse ordering
        # nconflicts
...
if __name__ == '__main__':
    ...
    # ===========
    write_css_stylesheet(css_filepath)
